    ...
    */
    public function getCount(int $user_id)
    {
        try{
                ...
                $response = array();
                $response['result'] = false;
                $response['error'] = get_class() . ':getCount() PDOException Error. Rollback was executed.' . $e->getMessage();
                Log::error(get_class() . ':getCount() PDOException Error. Rollback was executed.' . $e->getMessage());
                return $response;
            }
    }
